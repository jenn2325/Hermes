...
ggplot(df%>%group_by(Year),aes(x=Year_Q,y=Revenue,fill=Sector)) +scale_fill_brewer(palette = "Set2")+
  ...
  ggtitle("Revenue by sector from 2019 to 2020",subtitle = 'Quaterly') + ylab("Revenue in million of euros")+xlab("Year Quaterly")+geom_text(aes(y=pos,label=paste(percent, "%")),size=3)+theme(axis.title.y=element_text(angle=90,size=14),axis.title.x=element_text(size=14),plot.title = element_text(size=16),plot.subtitle = element_text(size=14))
```
<br>
...
q<-ggplot(returnmar,aes(y=return,x=Date,color=stock))+geom_line(size=0.6)+facet_grid(stock~.)+ggtitle('HERMES Stock Return vs Other in 2020 March')+theme_grey(8)+theme(legend.title=element_blank())+geom_point(data=mins,aes(x=Date,y=return,group=stock),size=1,color='black',text=return,label=Date)+labs(x='',y='')
f <- list(
  family = "Old Standard TT, serif",
  size = 5,
  color = "black"
...
See this plot from R gallery:https://www.r-graph-gallery.com/318-custom-dygraphs-time-series-example.html 
<br>
